...
Three epics are **complete**: the **design system** ([#5](https://github.com/oscar-ospina/saas-planner/issues/5)), the **consumer-app marketing-landing MVP** ([#16](https://github.com/oscar-ospina/saas-planner/issues/16), closed 2026-06-07), and **landing visual fidelity to Figma** ([#35](https://github.com/oscar-ospina/saas-planner/issues/35), closed 2026-06-07 — stories #36–#39 brought the Home hero/why/about sections to design fidelity). **▶ Active epic: [#31](https://github.com/oscar-ospina/saas-planner/issues/31) booking & checkout** — **PIVOT 2026-06-10: the Agenda shipped as a LOCAL MVP without Google Calendar** (story [#45](https://github.com/oscar-ospina/saas-planner/issues/45), **done & closed** via alta-vibracion-web PR #20): `/agenda` simulates availability client-side (1–3 random slots per viewed date, persisted in the visitor's `localStorage`), the visitor's booking persists too, and the details are handed to **WhatsApp** — **Liliana syncs her real calendar manually**. The spikes [#32](https://github.com/oscar-ospina/saas-planner/issues/32) + [#33](https://github.com/oscar-ospina/saas-planner/issues/33) ADRs (see "Booking ADRs" below) remain load-bearing for the **deferred real-integration track: #40 (GCal client merged; live smoke-test OWNER-GATED) → #41 → #43 → #44** (all Todo; #42's UI is essentially covered by #45 — only the availability source + Server Action submit remain). Resume that track **only when** the owner provisions GCP per `alta-vibracion-web/docs/booking-setup.md` and `npm run smoke:calendar` passes. *Pago* (Wompi) = Phase 2. Six ADRs are load-bearing and **not open for re-litigation** in a fresh session — read the relevant ones before proposing changes.
...
- **▶ Active — booking & checkout ([#31](https://github.com/oscar-ospina/saas-planner/issues/31)):** the **local-MVP Agenda is SHIPPED** ([story #45](https://github.com/oscar-ospina/saas-planner/issues/45), closed 2026-06-10, alta-vibracion-web PR #20): `/agenda` books **without a backend** — simulated availability (1–3 random «Reservado» slots per viewed date, `localStorage`, merge-on-write, PII-free), booking persists as «Tu cita», confirm hands the details to **WhatsApp**; hero + per-cita CTAs route to `/agenda`; **Liliana syncs her calendar manually**. **Deferred real-integration track (Todo):** [#40](https://github.com/oscar-ospina/saas-planner/issues/40) GCal client (merged; live smoke-test owner-gated) → [#41](https://github.com/oscar-ospina/saas-planner/issues/41) availability → [#43](https://github.com/oscar-ospina/saas-planner/issues/43) booking Server Action → [#44](https://github.com/oscar-ospina/saas-planner/issues/44) confirmation + `.ics` ([#42](https://github.com/oscar-ospina/saas-planner/issues/42)'s UI essentially covered by #45) — per the #32/#33 ADRs (service account + shared calendar + time-block events + `.ics`; no DB, GCal as source of truth, deterministic-id idempotency). **Phase 2 = *Pago*** (Wompi: card/PSE/Nequi, webhooks, email, DIAN factura) — that's what makes the app full-stack (would activate a future `api/` sibling). **▶ Next step = owner's call:** (a) resume the GCal track by running `alta-vibracion-web/docs/booking-setup.md` + `npm run smoke:calendar`, (b) start *Pago* Phase 2, (c) landing pre-launch polish (legal placeholders, real subdomain, OG image), or (d) the DS follow-ups #27–#30.
