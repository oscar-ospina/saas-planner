...
- **▶ Active — booking & checkout ([#31](https://github.com/oscar-ospina/saas-planner/issues/31)):** real in-app *Agenda*. **Phase 1 = booking-only via Google Calendar**, backend = Next.js Route Handlers/Server Actions **in `alta-vibracion-web`** (no `api/` sibling yet). **Phase 2 = *Pago*** (payment gateway card/PSE/Nequi, webhooks, email, DIAN factura) — that's what makes the app full-stack (would activate a future `api/` sibling). Run blocking spikes [#32](https://github.com/oscar-ospina/saas-planner/issues/32) (Google Calendar — auth/free-busy/event-create/slot-derivation) + [#33](https://github.com/oscar-ospina/saas-planner/issues/33) (backend & data — DB-or-none, double-book prevention, Vercel) → ADRs first. The MVP routes booking to WhatsApp until Phase 1 lands.
