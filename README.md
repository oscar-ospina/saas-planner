...
- ▶ **Active — booking & checkout** ([epic #31](https://github.com/oscar-ospina/saas-planner/issues/31)): real in-app *Agenda* — **Phase 1 = booking-only via Google Calendar** (backend = Next.js Server Actions/Route Handlers **in `alta-vibracion-web`**, no `api/` sibling yet); *Pago* = Phase 2. Spikes [#32](https://github.com/oscar-ospina/saas-planner/issues/32) (Google Calendar) + [#33](https://github.com/oscar-ospina/saas-planner/issues/33) (backend/data) are **resolved with ADRs** ([calendar](docs/superpowers/specs/2026-06-07-av-google-calendar.md), [backend](docs/superpowers/specs/2026-06-07-av-booking-backend.md)) — service account + shared calendar + time-block events + `.ics`; no DB, GCal as source of truth, deterministic-id idempotency. **Agenda Phase 1 stories #40–#44 are filed (Todo)** — #40 GCal client/setup/smoke-test → #41 availability/slots → #42 booking page → #43 booking Server Action → #44 confirmation + `.ics`; build next.
